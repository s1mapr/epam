...
import com.my.controllers.userController.servicesPayments.PhoneRechargeServlet;
...
public class PhoneRechargeTest {
    ...
    public void doGetTest() throws ServletException, IOException {
        final PhoneRechargeServlet servlet = new PhoneRechargeServlet();
        ...
        when(request.getRequestDispatcher("/views/jsp/options/phoneRecharge.jsp")).thenReturn(dispatcher);
        servlet.doGet(request, response);
        verify(session).removeAttribute("valid");
        verify(request).setAttribute("valid", validation);
        verify(session).setAttribute("purposeId", 1);
    ...
    public void doPostTest() throws ServletException, IOException {
        ...
        when(request.getParameter("phone")).thenReturn("+380976284216");
        when(request.getParameter("amount")).thenReturn("12");
        when(validation.phoneRechargeValidation("+380976284216", "12")).thenReturn(true);
        when(session.getAttribute("purposeId")).thenReturn(1);
        when(request.getParameter("accountId")).thenReturn("10");
        mockedAcc.when(()->AccountService.getCardId(10)).thenReturn(10);
        mockedCard.when(()->CardService.getAmount(10)).thenReturn(123.0);
        mockedRec.when(()->ReceiptService.createNewEntryInPhoneService("+380976284216")).thenReturn(123);
